...
def sql_add(title: str, count_skill: int):
    try:
        ...
        try:
            connection = psycopg2.connect(host='127.0.0.1', port=5432, user='hh_user', password='warlight123',
                                             database='hh')
            try:
...
def sql_del():
    try:
        connection = psycopg2.connect(host='127.0.0.1', port=5433, user='hh_user', password='warlight123',
                                      database='hh')
        try:
            with connection.cursor() as cursor:
